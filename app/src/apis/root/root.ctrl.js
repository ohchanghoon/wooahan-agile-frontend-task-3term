...
const todolist = {
    ...
    create: (req, res) => {
        ...
        const query =
            "INSERT INTO todos(id, name, description) VALUES (?, ?, ?);";

        db.query(
            query,
            [index, client.name, client.description],
            (err, result) => {
                if (err) throw err;
    ...
    update: (req, res) => {
        ...
        db.query(
            query,
            [client.name, client.description, index],
            (err, result) => {
                if (err) throw err;
...
const login = {
    login: (req, res) => {
        ...
        db.query(query, [client.id, client.password], (err, users) => {
            ...
            return res
                .status(200)
                .json({ msg: "success", code: users[0].code });
        });
    },
